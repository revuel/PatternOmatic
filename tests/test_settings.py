...
class TestConfig(unittest.TestCase):
    ...
    def test_xps_gop_can_not_be_enabled_together(self):
        ...
        super().assertNotEqual(
            config.use_grammar_operators, config.use_extended_pattern_syntax
        )
    ...
    def test_validate_config_argument(self):
        ...
        super().assertEqual(
            ...
            self.config._validate_config_argument(
                test_section, test_option_int, 1, config_parser
            ),
        )
        super().assertEqual(
            0.0,
            self.config._validate_config_argument(
                test_section, test_option_float, 0.1, config_parser
            ),
        )
        super().assertEqual(
            False,
            self.config._validate_config_argument(
                test_section, test_option_boolean, True, config_parser
            ),
        )
        super().assertEqual(
            "",
            self.config._validate_config_argument(
                test_section, test_option_string, "Whatever", config_parser
            ),
        )
        ...
        super().assertEqual(
            {},
            self.config._validate_config_argument(
                test_section, test_option_int, {}, config_parser
            ),
        )
